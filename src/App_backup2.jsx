...
import { 
  ...
  FileText, Mic, Save, MessageSquarePlus, Edit3, LogOut, Loader, ShieldCheck
} from 'lucide-react';
...
import { 
  getAuth, 
  signInWithPopup,       // 구글 팝업 로그인을 위해 추가
  GoogleAuthProvider,    // 구글 인증 제공자 추가
  onAuthStateChanged, 
  signOut
} from 'firebase/auth';
import { 
      ...
      <div className="bg-white max-w-md w-full rounded-3xl shadow-xl p-8 text-center">
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => onLogin('teacher')} className="w-full py-4 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold transition-all shadow-md flex items-center justify-center gap-2">
            <ShieldCheck size={20} /> 교사로 로그인 (Admin)
          </button>
        </div>
        <p className="text-xs text-slate-400 mt-8">Secure Login via Google & Firebase</p>
      </div>
    </div>
  );
};
